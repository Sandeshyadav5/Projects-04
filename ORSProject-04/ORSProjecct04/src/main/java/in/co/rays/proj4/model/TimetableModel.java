...
public class TimetableModel {
	...
	public void update(TimeTableBean bean) throws ApplicationException {
		...
		try {
			...
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn
					.prepareStatement("update st_timetable set semester=?, description=?, exam_date=?,  exam_time=?, course_id=?, course_name=?,subject_id=?,subject_name=?, created_by = ?, modified_by = ?, created_datetime = ?, modified_datetime = ? where id=?");
			...
			throw new ApplicationException("Exception in getting update subject");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
	...
	public void delete(TimeTableBean bean) throws ApplicationException {
		...
		try {
			...
			throw new ApplicationException("Exception in getting delete subject");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
	...
	public List search(TimeTableBean bean, int pageSize,int pageNo) throws ApplicationException   {
		...
		if(bean!=null) {
			...
			if(bean.getSemester()!=null && bean.getSemester().length()>0) {
				sql.append("and semester like this"+bean.getSemester()+"%");
			}
			if(bean.getDescription()!=null && bean.getDescription().length()>0) {
				sql.append("and Description like this"+bean.getDescription()+"%");
			}
			if(bean.getExamDate()!=null && bean.getExamDate().getDate()>0) {
				sql.append("and DoB like this"+bean.getExamDate()+"%");
			}
			if(bean.getExamTime()!=null && bean.getExamTime().length()>0) {
				sql.append("and ExamTime like this"+bean.getExamTime()+"%");
			}
			if(bean.getCourseId()>0) {
				sql.append("and CourseId like this"+bean.getCourseId()+"%");
			}
			if(bean.getCourseName()!=null && bean.getCourseName().length()>0) {
				sql.append("and CourseName like this"+bean.getCourseName()+"%");
			}
