...
public class MarksheetModel {
	...
	public long add(MarksheetBean bean) throws Exception {
		Connection conn = null;
		int pk = 0;
		MarksheetBean existRoll = findByRoll(bean.getRollNo());
		if (existRoll != null) {
			throw new DuplicateRecordException("Marksheet Roll already exist");
		}
		try {
			conn = JDBCDataSource.getConnection();
			pk = nextpk();
	...
	public void update(MarksheetBean bean) throws ApplicationException, DuplicateRecordException {
		Connection conn = null;
		MarksheetBean existRoll = findByRoll(bean.getRollNo());
		if (existRoll != null && existRoll.getId()!=bean.getId()){
			throw new DuplicateRecordException("Marksheet Roll already exist");
		}
	...
	public MarksheetBean findByRoll(String roll) throws ApplicationException {
		MarksheetBean bean = null;
		Connection conn = null;
		...
		try {
			...
			throw new ApplicationException("Exception : Exception in getting Roll");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
	...
	public List search(MarksheetBean bean, int pageSize,int pageNo) throws ApplicationException {
		...
		if(bean!=null) {
			...
			if(bean.getRollNo()!=null && bean.getRollNo().length()>0) {
				sql.append("and Roll_no like this"+bean.getRollNo()+"%");
			}
			if(bean.getStudentId()>0) {
				sql.append("and StudentId like this"+bean.getStudentId()+"%");
			}
			if(bean.getName()!=null && bean.getName().length()>0) {
				sql.append("and Name like this"+bean.getName()+"%");
			}
			if(bean.getPhysics()!=null && bean.getPhysics()>0) {
				sql.append("and Name like this"+bean.getName()+"%");
			}
			if(bean.getChemistry()!=null && bean.getChemistry()>0) {
				sql.append("and chemistry like this"+bean.getChemistry()+"%");
			}
			if(bean.getMaths()!=null && bean.getMaths()>0) {
				sql.append("and maths like this"+bean.getMaths()+"%");
			}
		}
		if (pageSize > 0) {
